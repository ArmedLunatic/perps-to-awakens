...
function TagBadge({ tag }: { tag: string }) {
  ...
  return (
    ...
    >
      ...
      <span>{tag}</span>
      {style.tooltip && (
        <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover/tooltip:block z-10 px-2 py-1 text-[10px] font-normal text-zinc-100 bg-zinc-800 border border-zinc-700 rounded whitespace-nowrap pointer-events-none">
          {style.tooltip}
        </span>
      )}
    </span>
...
function Tooltip({ children, content }: { children: React.ReactNode; content: string }) {
  return (
    <span className="group/tooltip relative inline-flex items-center">
      {children}
      <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover/tooltip:block z-20 px-2 py-1 text-[10px] font-normal text-zinc-100 bg-zinc-800 border border-zinc-700 rounded whitespace-nowrap pointer-events-none">
        {content}
      </span>
    </span>
  );
...
export default function EventTable({
  ...
  platformMode?: "strict" | "assisted" | "blocked";
}) {
          ...
          <tr className="border-b border-zinc-800/50 bg-zinc-900/30">
            ...
            {COLUMNS.map((col) => (
              <th
                ...
                className={`px-3 py-2.5 font-medium cursor-pointer hover:bg-zinc-800/30 select-none whitespace-nowrap transition-colors duration-150 ${
          ...
          {eventsToRender.map((item, i) => {
            ...
            return (
              <React.Fragment key={`${event.txHash}-${displayIndex}`}>
                <tr
                  className={`border-b border-zinc-800/30 transition-colors duration-150 ${
                    hasErrors
                ...
                >
                  ...
                  <td className="px-3 py-2.5 font-mono text-xs whitespace-nowrap text-zinc-400">
                    {event.date}
                  </td>
                  <td className="px-3 py-2.5 font-medium text-zinc-100">{event.asset}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-zinc-200">{formatNum(event.amount)}</td>
                  <td className="px-3 py-2.5 text-right font-mono text-zinc-400">{formatNum(event.fee)}</td>
                  <td className="px-3 py-2.5 text-right font-mono">
                    <PnLCell value={event.pnl} />
                  </td>
                  <td className="px-3 py-2.5 text-zinc-400">{event.paymentToken}</td>
                  <td className="px-3 py-2.5">
                    <TagBadge tag={event.tag} />
                  </td>
                  <td className="px-3 py-2.5 font-mono text-xs text-zinc-500 max-w-[180px] truncate" title={event.txHash}>
                    {event.txHash}
                  </td>
                </tr>
                {isExpanded && (
                  <tr className="bg-zinc-900/20 border-b border-zinc-800/20">
                      ...
                      <div className="space-y-2 text-xs">
                        ...
                        {hasErrors && (
                          <div className="pt-2 border-t border-red-800/30">
                            ...
                            <div className="space-y-0.5 font-mono text-red-400/90">
                        ...
                        {platformMode === "assisted" && (
                          <div className="pt-2 border-t border-amber-800/30">
                            <div className="flex items-center gap-1.5 text-amber-400/90">
                              ...
                              <span className="font-medium">Assisted Mode:</span> Review this event for accuracy
                            </div>
                          </div>
                        )}
                      </div>
  ...
  const categoryLabels: Record<EventCategory, { label: string; icon: string; color: string }> = {
    Trading: { label: "Trading", icon: "↔", color: "text-blue-400" },
    Income: { label: "Income", icon: "↑", color: "text-violet-400" },
    Expense: { label: "Expense", icon: "↓", color: "text-rose-400" },
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <label className="flex items-center gap-2 text-xs text-zinc-400 cursor-pointer">
            <input
              ...
              className="w-3.5 h-3.5 rounded border-zinc-700 bg-zinc-900 text-zinc-100 focus:ring-zinc-600"
            />
      ...
      {/* Table */}
      <div className="overflow-x-auto border border-zinc-800/50 rounded-lg bg-zinc-950/30">
        {groupByCategory ? (
                ...
                <div key={category}>
                  <div className="px-4 py-2.5 bg-zinc-900/40 border-b border-zinc-800/50">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-zinc-300">{categoryInfo.label}</span>
                      <span className="text-xs text-zinc-500">({categoryItems.length})</span>
                    </div>
                  </div>
